...
if __name__ == "__main__":
    ...
    with open(os.path.join(args.folder, files['teachers'])) as csvfile:
        ...
        for result in db_results:
            if result.import_id is not None:
                results_by_import[result.import_id] = result 
        ...
        # read in rows
        for row in reader:
            if row[csv_idx['import_id']] not in results_by_import:
                ...
                users_service.register_user(new_entry)
    ...
    with open(os.path.join(args.folder, files['students'])) as csvfile:
        ...
        # read in rows
        for row in reader:
            results = student_service.filter_students('student_id', row[csv_idx['student_id']])
            print results
            if len(results) is 0:
                new_entry = Student(
